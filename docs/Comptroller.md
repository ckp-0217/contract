...
```solidity
function _setCompSpeeds(contract CToken[] cTokens, uint256[] supplySpeeds, uint256[] borrowSpeeds) external nonpayable
```

Set COMP borrow and supply speeds for the specified markets.
...
| Name | Type | Description |
|---|---|---|
| cTokens | contract CToken[] | The markets whose COMP speed to update. |
| supplySpeeds | uint256[] | New supply-side COMP speed for the corresponding market. |
| borrowSpeeds | uint256[] | New borrow-side COMP speed for the corresponding market. |
...
```solidity
function fixBadAccruals(address[] affectedUsers, uint256[] amounts) external nonpayable
```

Delete this function after proposal 65 is executed
...
| Name | Type | Description |
|---|---|---|
| affectedUsers | address[] | undefined |
...
```solidity
function isDeprecated(contract CToken cToken) external view returns (bool)
```

Returns true if the given cToken market has been deprecated

*All borrows in a deprecated cToken market can be immediately liquidated*

#### Parameters

| Name | Type | Description |
|---|---|---|
| cToken | contract CToken | The market to check if deprecated |
...
| oldCompAccrued  | uint256 | undefined |
| newCompAccrued  | uint256 | undefined |

### CompBorrowSpeedUpdated

```solidity
event CompBorrowSpeedUpdated(contract CToken indexed cToken, uint256 newSpeed)
```
...
|---|---|---|
| user `indexed` | address | undefined |
| oldCompReceivable  | uint256 | undefined |
| newCompReceivable  | uint256 | undefined |

### CompSupplySpeedUpdated

```solidity
event CompSupplySpeedUpdated(contract CToken indexed cToken, uint256 newSpeed)
```
